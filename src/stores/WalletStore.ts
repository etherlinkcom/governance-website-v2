  ...
  async refreshVotingPower(): Promise<void> {
    ...
    try {
      ...
      if (delegates.length === 0) {
        const url = `https://api.tzkt.io/v1/delegates/${address}`; // TODO save string somwhere
      ...
      const delegateTotals = await Promise.all(
        delegates.map(async (delegateAddress) => {
          let staking = 0;
          try {
            const r = await fetch(`https://api.tzkt.io/v1/delegates/${delegateAddress}`);
            if (r.ok) {
