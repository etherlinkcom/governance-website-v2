...
import { Contract, TezosToolkit, TransactionWalletOperation } from '@tezos-x/octez.js';
...
export class WalletStore {
  ...
  private Tezos = new TezosToolkit(
    process.env.NEXT_PUBLIC_RPC_URL || 'https://rpc.tzkt.io/mainnet'
  );
  private wallet: BeaconWallet;
  private delegatesViewContractAddress: string =
    process.env.NEXT_PUBLIC_VOTING_RIGHTS_DELEGATION_CONTRACT || 'KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH';
  private readonly tzktApiUrl: string =
    process.env.NEXT_PUBLIC_TZKT_API_URL || 'https://api.tzkt.io/v1';
  ...
  get allVoterAddresses(): string[] {
    return [this._alias, ...Array.from(this.delegates.keys())].filter(
  ...
  private updateCanVoteOnAddresses(delegations: Map<string, DelegationRule>) {
    ...
    const activeContracts: string[] = Object.values(contractStore['activeContracts'] || {}).map(
  ...
  async connect(): Promise<void> {
    ...
    await Promise.all([this.refreshBalance(), this.refreshVotingPower()]);
    ...
    try {
          ...
          delegates.map(async (delegateAddress) => {
            try {
              const response: Response = await fetch(
                `${this.tzktApiUrl}/voting/periods/current/voters/${delegateAddress}`
              );
              ...
              const data: { votingPower?: number; delegate: { alias?: string } } =
                await response.json();
      ...
      runInAction(() => {
        this.votingPower = {
          ...
          votingPercent: totalVotingPower.isZero()
            ? null
            : aggregatedVotingPower.div(totalVotingPower).times(100),
        };
      });
    } catch (error) {
  ...
  async vote(contractAddress: string, voteType: VoteOption): Promise<OperationResult | undefined> {
    ...
    try {
      ...
      const confirmation: TransactionOperationConfirmation | undefined =
        await operation.confirmation();
        
      toast.success(`Successfully voted ${voteType}`);
      return {
        opHash: operation.opHash,
        level: confirmation?.block.header.level,
  ...
  async upvoteProposal(
    ...
    try {
      ...
      const operation = await contract.methodsObject
        .upvote_proposal(sequencerProposal ?? proposal)
        .send();
      const confirmation: TransactionOperationConfirmation | undefined =
        await operation.confirmation();
  ...
  async submitProposal(
    ...
    try {
      const contract = await this.Tezos.wallet.at(contractAddress);
      const operation: TransactionWalletOperation = await contract.methodsObject
